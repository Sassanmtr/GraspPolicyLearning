...
def vis_target_actual_gt(actual_file, target_file):
    ...
    # gt_positions = np.array([ee_info[i].t for i in range(len(ee_info))])

    # Plot the positions as points in 3D space
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    ax.scatter(target_positions[:,0], target_positions[:,1], target_positions[:,2], color='b', label='robot path')
    ax.scatter(actual_positions[:,0], actual_positions[:,1], actual_positions[:,2], color='r', label='collected_data')
    # ax.scatter(gt_positions[:,0], gt_positions[:,1], gt_positions[:,2], color='g', label='gt')
...
def vis_action_gt(traj_dir, replay_memory):
    ee_info = []
    ...
    for i in range(action.shape[0]):
        ee_processed.append(output_processing(ee_processed[i], action[i]))
    # Extract the positions from the dictionaries
    target_positions = np.array([ee_processed[i].t for i in range(len(ee_processed))])
    gt_positions = np.array([ee_info[i].t for i in range(len(ee_info))])
    ...
    ax.scatter(target_positions[:,0], target_positions[:,1], target_positions[:,2], s=2, color='b', label='action')
    ax.scatter(gt_positions[:,0], gt_positions[:,1], gt_positions[:,2], s=2, color='r', label='gt')
...
pose_file = np.load(traj_dir + "/pose.npy", allow_pickle=True)
corres_file = np.load(data_dir + "/corresponding_ee_pose.npy", allow_pickle=True)
...
# for i in range(200):
#     ee_info.append(pose_file.item()[i]["ee_pose"])
#     corres_info.append(corres_file.item()[i])
...
# pos_rotation_visualization(corres_info)
...
# vis_target_actual_gt(actual_file, target_file)
# vis_target_actual_gt(actual_file=corres_info, target_file=ee_info)
vis_action_gt(traj_dir)