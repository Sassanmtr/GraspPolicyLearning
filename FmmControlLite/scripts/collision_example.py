...
s1 = sg.Cylinder(radius=0.135, length=1.5, pose=sm.SE3(0.1, 0.000, 0.930))
s1.v = [0, 0, 0, 0, 0, 0]

collisions = [s1]

# Make a target
target = sg.Sphere(radius=0.02, pose=sm.SE3(1.6, -0.2, 0.0))
...
def step():
    ...
    for collision in collisions:
        ...
        c_Ain, c_bin = link_collision_damper(panda,
            collision,
            panda.q[:n],
            0.3,
            0.01,
    ...
    lb = np.r_[panda.qd_lb[:n], -10 * np.ones(6)]
    ub = np.r_[panda.qd_ub[:n], 10 * np.ones(6)]

    # Solve for the joint velocities dq
    qd = qp.solve_qp(Q, c, Ain, bin, Aeq, beq, lb=lb, ub=ub, solver="osqp")

    if qd is None:
        print("qd is none, skip")
    else:
